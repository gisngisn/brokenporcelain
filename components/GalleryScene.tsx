...
const artworks: Artwork[] = [
  {
    ...
    title: "Qinghuan",
    description: "A fragile porcelain memory preserved in silence.",
  },
  {
    id: "002",
...
export default function GalleryScene() {
  ...
  const previous = () => {
    setCurrent((value) => (value === 0 ? artworks.length - 1 : value - 1));
  };

  const next = () => {
    setCurrent((value) => (value === artworks.length - 1 ? 0 : value + 1));
  };
  ...
  return (
    ...
    >
      ...
      <KeyboardGallery onPrevious={previous} onNext={next} />
      ...
      <div
        ...
        "
      >
        <ArtworkViewer key={artwork.id} image={artwork.image} title={artwork.title} />

        <ArtworkInfo title={artwork.title} description={artwork.description ?? ""} />
      </div>
